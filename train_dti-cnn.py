...
from models.models import BCL_CSS
from utils import *
from Sample_processing import *

###############################################################
# Training settings
parser = argparse.ArgumentParser(description='BCL-CSS')
...
parser.add_argument('--model_type', default="pretrain_drug", type=str, help="the model type decide which model to run")

parser.add_argument('--alpha1', default=0.05, type=float, help='init negative sample threshold')
parser.add_argument('--alpha2', default=0.95, type=float, help='drug batch size')
...
def train(args,epoch,dti_train_index, dti_train_label,dti_train_pos_mat,ddi_train_index, ddi_train_label,ppi_train_index, ppi_train_label,
    ...
    if args.model_type == "pretrain_drug":
        ...
        output, loss1 = model(drug_data, protein_data, ddi_train_index, ppi_train_index, dti_train_index,
                                                      dti_train_label, dti_train_pos_mat)
...
for i in range(5):
    ...
    """
    if args.model_type == "pretrain_drug" or args.model_type == "all_module":
    ...
    if args.model_type == "pretrain_protein" or args.model_type == "all_module":
    ...
    for fold_num in range(args.fold_num):
        ...
        model = BCL_CSS(args,ddi_train_pos,ppi_train_pos,drug_embs,protein_embs,DrugSimNet,ProteinSimNet,DrugSimNet_ori,ProteinSimNet_ori)
