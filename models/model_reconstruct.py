...
from models.CGAT import GATConv as CGATConv
...
class model_reconstruct(nn.Module):
    ...
    def __init__(self, args, drug_emb, protein_emb,DrugSimNet, ProteinSimNet,DrugSimNet_ori,ProteinSimNet_ori):
        ...
        negative_slope = 0.2
        dropout = 0.2
        self.cgat1_drug = CGATConv(args.drug_dim, self.gcn_drug_outdim,nhead,True,negative_slope,dropout)
        self.cgat2_drug = CGATConv(args.drug_dim, self.gcn_drug_outdim, nhead, True, negative_slope,dropout)
        self.cgat3_drug = CGATConv(args.drug_dim, self.gcn_drug_outdim, nhead, True, negative_slope,dropout)
        self.cgat4_drug = CGATConv(args.drug_dim, self.gcn_drug_outdim, nhead, True, negative_slope,dropout)
        self.cgat1_protein = CGATConv(args.protein_dim, self.gcn_protein_outdim,nhead, True, negative_slope,dropout)
        self.cgat2_protein = CGATConv(args.protein_dim, self.gcn_protein_outdim,nhead, True, negative_slope,dropout)
        self.cgat3_protein = CGATConv(args.protein_dim, self.gcn_protein_outdim,nhead, True, negative_slope,dropout)

        """第二层CGAT"""
        self.cgat12_drug = CGATConv(self.gcn_drug_outdim*nhead, self.gcn_drug_outdim,1, False, negative_slope,dropout)
        self.cgat22_drug = CGATConv(self.gcn_drug_outdim*nhead, self.gcn_drug_outdim,1, False, negative_slope,dropout)
        self.cgat32_drug = CGATConv(self.gcn_drug_outdim*nhead, self.gcn_drug_outdim,1, False, negative_slope,dropout)
        self.cgat42_drug = CGATConv(self.gcn_drug_outdim*nhead, self.gcn_drug_outdim,1, False, negative_slope,dropout)
        self.cgat12_protein = CGATConv(self.gcn_protein_outdim*nhead, self.gcn_protein_outdim,1,False, negative_slope,dropout)
        self.cgat22_protein = CGATConv(self.gcn_protein_outdim*nhead, self.gcn_protein_outdim,1,False, negative_slope,dropout)
        self.cgat32_protein = CGATConv(self.gcn_protein_outdim*nhead, self.gcn_protein_outdim,1,False, negative_slope,dropout)
    ...
    def get_contrast_pair(self,args, feat_sim,sim_net):
        ...
        neg_num = int(max_neg_num * current_epoch*0.8 / total_epoch) + 1
        ...
        # 获得feature sim中的正样本
        if (not feat_sim is None) and (current_epoch >=10000):
            vals_fea, indices_fea = feat_sim.topk(k=pos_num, dim=1, largest=True, sorted=True)
            sim_drug_index_row_fea = torch.arange(len(feat_sim), dtype=torch.int32).repeat_interleave(pos_num,dim=0).unsqueeze(0)
            ...
            pos_fea = torch.zeros_like(feat_sim)
            ...
            #　取计算得到的相似度值
            num_sim = torch.where(pos > 0, 1, 1e-6) + torch.where(pos_fea > 0, 1, 1e-6)
            pos = pos / num_sim
    ...
    def forward(self,dti_index,dti_labels,train_pos_dti,drug_emb=None,protein_emb=None):
        ...
        """第一层CGAT"""
        ## todo 使用CGAT
        drug_embs_1 = self.cgat1_drug(drug_emb, d_edge_index_1,alpha_ori=d_alpha_ori_1)
        drug_embs_2 = self.cgat2_drug(drug_emb, d_edge_index_2, alpha_ori=d_alpha_ori_2)
        drug_embs_3 = self.cgat3_drug(drug_emb, d_edge_index_3, alpha_ori=d_alpha_ori_3)
        drug_embs_4 = self.cgat4_drug(drug_emb, d_edge_index_4, alpha_ori=d_alpha_ori_4)
        protein_embs_1 = self.cgat1_protein(protein_emb, p_edge_index_1, alpha_ori=p_alpha_ori_1)
        protein_embs_2 = self.cgat2_protein(protein_emb, p_edge_index_2, alpha_ori=p_alpha_ori_2)
        protein_embs_3 = self.cgat3_protein(protein_emb, p_edge_index_3, alpha_ori=p_alpha_ori_3)

        """
        第二层CGAT 
        """
        drug_embs_12 = self.cgat12_drug(drug_embs_1, d_edge_index_1, alpha_ori=d_alpha_ori_1)
        drug_embs_22 = self.cgat22_drug(drug_embs_2, d_edge_index_2, alpha_ori=d_alpha_ori_2)
        drug_embs_32 = self.cgat32_drug(drug_embs_3, d_edge_index_3, alpha_ori=d_alpha_ori_3)
        drug_embs_42 = self.cgat42_drug(drug_embs_4, d_edge_index_4, alpha_ori=d_alpha_ori_4)

        protein_embs_12 = self.cgat12_protein(protein_embs_1, p_edge_index_1, alpha_ori=p_alpha_ori_1)
        protein_embs_22 = self.cgat22_protein(protein_embs_2, p_edge_index_2, alpha_ori=p_alpha_ori_2)
        protein_embs_32 = self.cgat32_protein(protein_embs_3, p_edge_index_3, alpha_ori=p_alpha_ori_3)
