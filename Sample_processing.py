...
def gen_train_data0809(dti_train_index, dti_train_label,dti_test_index, drug_num, protein_num,device):

    test_mask = np.ones(drug_num * protein_num)
    dti_train_index = np.array(dti_train_index.cpu())
    dti_test_index = np.array(dti_test_index.cpu())
    dti_train_label = np.array(dti_train_label.cpu())

    """获得除该fold_num之外的正样本，组合成训练集正样本"""

    # pos_index = np.concatenate((dti_train_index, dti_test_index), axis=-1)
    pos_index = dti_train_index[:,np.where(dti_train_label==1)[0]]
    index_id = pos_index[0, :] * protein_num + pos_index[1, :]-drug_num
    test_mask[index_id] = 0

    test_index_id = dti_test_index[0, :] * protein_num + dti_test_index[1, :]-drug_num
    test_mask[test_index_id] = 0
    ...
    drug_id = np.floor_divide(id_chosen, protein_num)
    protein_id = np.mod(id_chosen, protein_num)
    neg_index = np.array([drug_id, protein_id+drug_num]) # todo protein id 从drug id之后开始
